...
export async function getSecret(secretId: string | null | undefined): Promise<string | null> {
  ...
  if (error) throw new Error(`Falha ao ler segredo do Vault: ${error.message}`)
  return (data as string) ?? null
}

/** Cria/atualiza um segredo no Vault e retorna o id. */
export async function setSecret(name: string, secret: string): Promise<string> {
  const { data, error } = await db.rpc('vault_set_secret', { p_name: name, p_secret: secret })
  if (error) throw new Error(`Falha ao gravar segredo no Vault: ${error.message}`)
