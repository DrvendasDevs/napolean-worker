...
export function buildUserContext(ctx: EvaluationContext): string {
  return [
...
function tryParse(raw: string): { ok: true; value: Evaluation } | { ok: false; error: string } {
  let json: unknown
  try {
    json = JSON.parse(raw)
  } catch {
