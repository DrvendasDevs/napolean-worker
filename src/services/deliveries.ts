...
import type { DeliveryStatus } from '../shared/stages.js'

export type ArtifactType = 'summary' | 'report_pdf' | 'transcription_pdf' | 'original_audio'

export interface Recipient {
  role: 'napoleon' | 'gestor' | 'vendedor'
  userId: string | null
  phone: string
}
...
export interface DeliveryArtifacts {
  summaryText: string
  reportPdfPath: string | null // caminho no bucketArtifacts
  transcriptionPdfPath: string | null
  originalAudio: { path: string; sizeBytes: number } | null // bucketMedia
  fileName: string
}

interface ArtifactConfig {
  send_summary: boolean
  send_report_pdf: boolean
  send_transcription_pdf: boolean
  send_original_audio: boolean
  audio_size_limit_mb: number
}

async function getArtifactConfig(): Promise<ArtifactConfig> {
...
export async function resolveRecipients(analysis: {
  ...
  if (napoleonPhone) recipients.push({ role: 'napoleon', userId: null, phone: napoleonPhone })
  ...
  let sellerManagerId: string | null = null
  if (transc?.usuario_avaliado_id) {
    ...
    if (seller) {
      ...
      if (p) recipients.push({ role: 'vendedor', userId: seller.id, phone: p })
    }
  }
  ...
  if (sellerManagerId) {
    ...
    if (manager) {
      ...
      if (p) recipients.push({ role: 'gestor', userId: manager.id, phone: p })
    }
  }

  // Dedupe por telefone, agregando papéis (não duplica mensagens).
  const byPhone = new Map<string, Recipient & { roles: string[] }>()
...
export async function ensureDeliveries(
  analysisId: string,
  recipients: (Recipient & { roles?: string[] })[],
  artifacts: DeliveryArtifacts,
): Promise<void> {
  const cfg = await getArtifactConfig()
  const enabled: ArtifactType[] = []
  if (cfg.send_summary) enabled.push('summary')
  if (cfg.send_report_pdf && artifacts.reportPdfPath) enabled.push('report_pdf')
  if (cfg.send_transcription_pdf && artifacts.transcriptionPdfPath) enabled.push('transcription_pdf')
  if (
    cfg.send_original_audio &&
  ...
  for (const r of recipients) {
    ...
    const artifactsForRecipient: ArtifactType[] = isSellerOrManager ? ['summary'] : enabled
...
export async function sendPendingDeliveries(
  ...
  for (const d of deliveries ?? []) {
    ...
    try {
      let resp: { ok: boolean; status: number; json: unknown }
      ...
      if (resp.ok) {
        await db
          ...
          .update({
            ...
            zapi_response: resp.json as object,
            error_code: null,
            error_message: null,
          })
          .eq('id', d.id)
