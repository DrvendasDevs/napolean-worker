...
export async function probe(filePath: string): Promise<AudioInfo> {
  ...
  if (code !== 0) throw new Error(`ffprobe falhou: ${stderr}`)
  const json = JSON.parse(stdout)
  const audioStream = (json.streams || []).find((s: { codec_type: string }) => s.codec_type === 'audio')
  if (!audioStream) throw new Error('Nenhuma faixa de áudio encontrada no arquivo')
  return {
 ...
 */
export async function normalizeForSpeech(inputPath: string, outputDir: string): Promise<string> {
...
): Promise<{ chunkNumber: number; path: string; startMs: number; endMs: number }[]> {
  await fs.mkdir(outputDir, { recursive: true })
  const chunkSec = config.audioChunkSeconds
  const overlap = config.audioChunkOverlapSeconds
  const chunks: { chunkNumber: number; path: string; startMs: number; endMs: number }[] = []
