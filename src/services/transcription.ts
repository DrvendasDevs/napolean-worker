 ...
 */
export async function transcribePendingChunks(analysisId: string, client: OpenAI): Promise<void> {
  ...
  if (error) throw new Error(`Falha ao listar chunks: ${error.message}`)
...
export async function allChunksDone(analysisId: string): Promise<boolean> {
...
export async function consolidateTranscript(analysisId: string): Promise<string> {
  const { data: chunks, error } = await db
    .from('audio_chunks')
    .select('chunk_number, start_ms, transcription_text')
    .eq('analysis_id', analysisId)
    .order('chunk_number', { ascending: true })
  if (error) throw new Error(`Falha ao consolidar transcrição: ${error.message}`)
