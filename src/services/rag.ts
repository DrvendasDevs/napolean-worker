...
export function chunkText(text: string): { content: string; position: number }[] {
...
export async function buildDocumentContext(
  ...
  for (const stage of STAGES) {
    ...
    const { data, error } = await db.rpc('match_document_chunks', {
      ...
      p_query_embedding: qVec as unknown as string,
      p_match_count: 4,
    })
    if (error) throw new Error(`match_document_chunks falhou: ${error.message}`)
