 ...
 */
export async function processIndexDocument(job: Job): Promise<void> {
  const versionId = (job.metadata?.version_id as string) || null
  if (!versionId) throw new Error('Job index_document sem version_id')
  ...
  const { data: version, error } = await db
    ...
    .eq('id', versionId)
    .single()
  if (error || !version) throw new Error(`Versão de documento ${versionId} não encontrada`)
  ...
  try {
    ...
    if (!text.trim()) throw new Error('Documento sem texto extraível')
    ...
    const refCol =
      version.document_type === 'documento_mestre'
...
async function extractDocumentText(localPath: string, mime: string, tmp: string): Promise<string> {
