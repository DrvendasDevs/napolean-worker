...
import { setJobStage, completeJob } from './queue.js';
import { ensureTmp, cleanupTmp, downloadToFile, uploadFile, uploadBuffer, sha256File, } from './services/storage.js';
import { probe, normalizeForSpeech, splitIntoChunks, fileSize } from './services/ffmpeg.js';
import { transcribePendingChunks, allChunksDone, consolidateTranscript, } from './services/transcription.js';
...
import { buildReportMarkdown, buildSummary, generateReportPdf, generateTranscriptionPdf, resolveOverallImprovements, resolveOverallStrengths, } from './services/report.js';
import { resolveRecipients, ensureDeliveries, sendPendingDeliveries, } from './services/deliveries.js';
import { STAGE_TO_COLUMN, computeFinalScore, scoresFromEvaluation, } from './shared/evaluation.js';
function nowBr() {
...
export async function processAnalysis(job) {
    ...
    if (error || !analiseData)
        throw new Error(`Análise ${analysisId} não encontrada`);
    const analise = analiseData;
    const workspaceId = analise.workspace_id;
    if (!workspaceId)
        throw new Error('Análise sem workspace_id');
    ...
    const isDocx = lowerName.endsWith('.docx') || mime.includes('officedocument.wordprocessingml');
...
// ------------------------- Helpers -------------------------
async function getExistingTranscription(analysisId) {
...
async function handleAudio(job, analise, originalPath, tmpBase, ctx) {
...
async function handlePdf(job, analise, originalPath, tmpBase, ctx) {
    ...
    if (performedOcr)
        logger.info('pdf.ocr.done', { ...ctx, processing_stage: 'performing_ocr' });
    return finalTexts.join('\n\n').trim();
}
/** Congela versões ativas de prompt/documentos e modelo (uma única vez). */
async function freezeVersions(analise) {
    const { data: cfg } = await db
        .from('agentes_config')
        .select('active_prompt_version_id, active_documento_mestre_version_id, active_script_nota_10_version_id, model_provider, model_name')
    ...
    const promptVersionId = analise.prompt_version_id ??
        cfg?.active_prompt_version_id ??
        (await activePromptVersionId(analise.workspace_id));
    const docMestreId = analise.documento_mestre_version_id ??
        cfg?.active_documento_mestre_version_id ??
        (await activeDocVersionId(analise.workspace_id, 'documento_mestre'));
    const scriptId = analise.script_nota_10_version_id ??
        cfg?.active_script_nota_10_version_id ??
        (await activeDocVersionId(analise.workspace_id, 'script_nota_10'));
    if (!promptVersionId)
        throw new Error('Nenhum prompt ativo configurado para o workspace');
    if (!docMestreId)
        throw new Error('Documento Mestre não configurado — avaliação não iniciada');
    if (!scriptId)
        throw new Error('Script Nota 10 não configurado — avaliação não iniciada');
    ...
    const { data: prompt } = await db
        ...
        .single();
    if (!prompt)
        throw new Error('Versão de prompt não encontrada');
...
async function activePromptVersionId(workspaceId) {
    const { data } = await db
        .from('agent_prompt_versions')
        .select('id')
...
async function activeDocVersionId(workspaceId, tipo) {
    const { data } = await db
        .from('agent_document_versions')
        .select('id')
        .eq('workspace_id', workspaceId)
...
async function getDocText(versionId) {
...
async function gatherMeta(analysisId, workspaceId) {
...
async function getExistingEvaluation(analysisId) {
    const { data } = await db.from('relatorios').select('avaliacao_json').eq('analise_id', analysisId).maybeSingle();
    return data?.avaliacao_json ?? null;
}
async function persistReport(analysisId, workspaceId, evaluation, scores, finalScore, markdown) {
    const row = {
